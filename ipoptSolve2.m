...
funcs.gradient = @(z) diagTwoH.*z + g + sparse(Ps')*calcDs2(z,Ps,hs,rho,flagNoPade);
funcs.constraints = @(z) [sparse(C)*z; sparse(P)*z];
options.cu = [b; h];
options.cl = [b; h-inf];% equality constraints Cz=b; ineqs Pz<=h
funcs.jacobian = @(z) [sparse(C); sparse(P)];
funcs.jacobianstructure = @() [sparse(C); sparse(P)];
funcs.hessian = @(z,sigma,lambda) hessian(z,sigma,lambda,H,P,h,Ps,hs,rho,kappa,ns,ni);
funcs.hessianstructure = @() sparse(H) + sparse(Ps)'*sparse(Ps);
...
hess_full = sigma*(sparse(2*H) + rho*sparse(Ps)'*spdiags(es ./ ((1+es).*(1+es)),0,ns,ns)*sparse(Ps));
hess = tril(hess_full);
